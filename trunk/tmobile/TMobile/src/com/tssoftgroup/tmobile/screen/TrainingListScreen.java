 ...
 * UiApplication.
 */
public class TrainingListScreen extends FixMainScreen implements FieldChangeListener{
	...
	public void setList(Vector trainInfos) {
		...
		for (int i = 0; i < trainInfos.size(); i++) {
			_list.add((TrainingInfo) trainInfos.elementAt(i));
			all.addElement((TrainingInfo) trainInfos.elementAt(i));
		}
	}
	...
	public void createVideoMain() {
		...
		try {
			...
			// EditField
			searchTextField = new EditFieldwithFocus("Search: ", topic, 35,
...
//			  bf.setMargin(edge);
//			videoManager.add(bf);
			LabelField topicLabel = new LabelFieldWithFullBG("Training",MyColor.FONT_TOPIC,MyColor.FONT_TOPIC_COLOR,MyColor.TOPIC_BG,Const.LABEL_WIDTH);
