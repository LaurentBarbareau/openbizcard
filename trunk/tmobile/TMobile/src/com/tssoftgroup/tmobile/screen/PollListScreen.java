 ...
 * UiApplication.
 */
public class PollListScreen extends FixMainScreen implements FieldChangeListener{
	Img imgstock = Img.getInstance();
	private MainItem _mainMenuItem = new MainItem();
	String topic = "";
	boolean alreadyPush = false;
	static PollListScreen instance;
	...
	public static PollListScreen getInstance() {
		if (instance == null) {
			instance = new PollListScreen();
		}
		return instance;
	}

	private PollListScreen() {
		super(MODE_POLL);
	...
	public PollListScreen(String topic) {
		super(MODE_POLL);
		nextBT.setChangeListener(this);
		previousBT.setChangeListener(this);
		this.topic = topic;
	...
	public void setList(Vector pollInfos) {
		_list.removeAll();
		all.removeAllElements();
		for (int i = 0; i < pollInfos.size(); i++) {
			_list.add((PollInfo) pollInfos.elementAt(i));
			all.addElement((PollInfo) pollInfos.elementAt(i));
		}
	}
	...
	private void handleSearchType() {
		if (searchTextField.isFocus) {
			...
			for (int i = 0; i < all.size(); i++) {
				PollInfo picInfo = (PollInfo) all.elementAt(i);
	...
	private void selectAction() {
		if (_list != null && _list.getSize() > 0) {
			...
			if (_list.get(_list, _list.getSelectedIndex()) != null) {
				PollInfo pollInfo = (PollInfo) _list.get(_list, _list
						.getSelectedIndex());
				if (pollInfo != null) {
					UiApplication.getUiApplication().pushScreen(
							WaitScreen.getInstance());
					Engine.getInstance().getPollChoice(pollInfo);
					// PollQuestionScreen scr = new
					// PollQuestionScreen(pollInfo);
	...
	public void createVideoMain() {
		...
		try {
			...
			// videoManager.add(bf);
			LabelField topicLabel = new LabelFieldWithFullBG("Company Poll",
