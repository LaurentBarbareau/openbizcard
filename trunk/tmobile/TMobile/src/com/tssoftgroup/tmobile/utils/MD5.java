...
public class MD5 {
	...
	private final void transform(MD5State state, byte buffer[], int shift,
			int[] decode_buf) {
	 ...
	 *            buffer.length)
	 */
	private final void update(MD5State stat, byte buffer[], int offset,
			int length) {
	...
	public static String md5(String input) {
		...
		// byte md5[] = md5.fingerprint(plain);
		return hashResult;
	}
}
