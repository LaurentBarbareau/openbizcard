...
public class WebServiceReaderMyTeam {
	...
	public static String getDeviceId(Activity act) {
		if (mTelephonyMgr == null) {
			mTelephonyMgr = (TelephonyManager) act.getSystemService(Activity.TELEPHONY_SERVICE);
	...
	public static ArrayList<ArticleBySubject> getUserArrticles(String userId)
		...
		try {
			...
			for (int i = 0; i < resultRequestSOAP.getPropertyCount(); i++) {
				...
				SoapObject articleBySubject = (SoapObject) resultRequestSOAP.getProperty(i);
				SoapPrimitive subject = (SoapPrimitive) articleBySubject.getProperty("Subject");
				SoapObject articles = (SoapObject) articleBySubject.getProperty("Articles");
				ArrayList<Article> articlesArr = new ArrayList<Article>();
				for (int j = 0; j < articles.getPropertyCount(); j++) {

					SoapObject article = (SoapObject) articles.getProperty(j);
					SoapPrimitive id = (SoapPrimitive) article.getProperty("ID");
					SoapPrimitive title = (SoapPrimitive) article.getProperty("Title");
					SoapPrimitive scTitle = (SoapPrimitive) article.getProperty("sc_Title");
					SoapPrimitive imageUrl = (SoapPrimitive) article.getProperty("ImageURL");
					SoapPrimitive isHighlight = (SoapPrimitive) article.getProperty("IsHighlighted");
