...
public class WebServiceReader {
	...
	public static Article getArticleByID(Article article, String indexType) {
		...
		try {
			androidHttpTransport.call(SOAP_ACTION, envelope);
			resultRequestSOAP = (SoapObject) envelope.getResponse();
			SoapPrimitive body = (SoapPrimitive) resultRequestSOAP
					.getProperty("Body");
			SoapPrimitive nextId = (SoapPrimitive) resultRequestSOAP
					.getProperty("NextID");
			SoapPrimitive prevId = (SoapPrimitive) resultRequestSOAP
					.getProperty("PrevID");
			Log.v(TAG, "Response from servcer:" + resultRequestSOAP.toString());
