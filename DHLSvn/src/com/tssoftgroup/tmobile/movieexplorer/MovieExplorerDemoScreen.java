 ...
 */
/* package */public class MovieExplorerDemoScreen extends MainScreen {
	private TMobile _uiApp;
	private MovieExplorerDemoListFieldImpl _list;
	private MovieExplorerDemoJournalListener _fileListener;
	 ...
	 * Constructor
	 */
	public MovieExplorerDemoScreen() {
		setTitle("Share a video");

		_list = new MovieExplorerDemoListFieldImpl();
		add(_list);
		readRoots(null);

		_uiApp = (TMobile) UiApplication.getUiApplication();
		_fileListener = new MovieExplorerDemoJournalListener(this);
		_uiApp.addFileSystemJournalListener(_fileListener);
	}

	public MovieExplorerDemoScreen(TextField tf) {
		this.tf = tf;
		setTitle("Share a video");
	...
	private void deleteAction() {
		...
		MovieExplorerDemoFileHolder fileholder = (MovieExplorerDemoFileHolder) _list
				.get(_list, index);
	...
	private void readSubroots(String file) {
		...
		try {
			...
			MovieExplorerDemoFileHolder fileholder = new MovieExplorerDemoFileHolder(
					file);
			fileholder.setDirectory(fc.isDirectory());
			if (fc.isDirectory()
					|| (!fc.isDirectory() && file.toLowerCase()
							.endsWith(".3gp"))) {
	...
	private boolean selectAction() {
		...
		if (fileholder != null) {
			...
			if (fileholder.isDirectory()) {
				...
				Const.videoToUpload = fileholder.getPath()
